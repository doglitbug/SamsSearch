...
Assets::Assets()
{
    ...
    // Create music tracks
    m_pTitleMusicTrack = MIX_CreateTrack(m_pMixer);
    loadMusic("assets/music/chiptunes/CGE-title-theme.flac", "main_menu");
    MIX_SetTrackAudio(m_pTitleMusicTrack, m_music["main_menu"]);

    m_pGameMusicTrack = MIX_CreateTrack(m_pMixer);
    // Create sound track
                                    ...
                                    const std::vector<std::string> &dialog, const std::string &fontID)
{
    ...
    // Save to Textures
    // TODO Do we need to delete existing texture here?
...
void Assets::toggleTitleMusic(bool enabled)
{
    if (enabled && App::get()->getSettings()->getTitleMusicEnabled())
    {
        MIX_PlayTrack(m_pTitleMusicTrack, -1);
    }
    else
    {
        MIX_StopTrack(m_pTitleMusicTrack, 0);
    }
}

void Assets::setTitleMusicVolume(int volume)
{
    MIX_SetTrackGain(m_pTitleMusicTrack, static_cast<float>(volume) / 100.0f);
}

void Assets::playGameMusic(const std::string &id, int loop)
{
    MIX_SetTrackAudio(m_pGameMusicTrack, m_music[id]);
    if (!App::get()->getSettings()->getGameMusicEnabled())
        return;
    MIX_PlayTrack(m_pGameMusicTrack, -1);
}

void Assets::toggleGameMusic(bool enabled)
{
    if (enabled && App::get()->getSettings()->getGameMusicEnabled())
    {
        MIX_ResumeTrack(m_pGameMusicTrack);
    }
    else
    {
...
void Assets::onNotify(const std::string &message, MyType newValue)
{
    if (message == "TITLE_MUSIC_ENABLED")
    {
        toggleTitleMusic(std::get<bool>(newValue));
    }

    if (message == "TITLE_MUSIC_VOLUME")
    {
        setTitleMusicVolume(std::get<int>(newValue));
    }
