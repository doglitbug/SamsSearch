...
void PlayState::render()
{
    ...
    for (BaseLayer *layer : *pCurrentMap->getLowerLayers())
    {
        layer->render(&pViewport);
    }
    for (GameObject *gameObject : *pCurrentMap->getGameObjects())
    {
        gameObject->drawSelf(&pViewport);
    }
    // Draw player on top of everyone else
    mPlayer->drawSelf(&pViewport);
    for (BaseLayer *layer : *pCurrentMap->getUpperLayers())
    {
        layer->render(&pViewport);
    }
